...
def tweet(message, im=False):
	...
	if im: api.update_with_media("image.png", str(message)) # Sends message and attachemnt as tweet
	else:
		try: api.update_status(str(message)) # Sends message as tweet
		except: print("error lol")
...
async def on_message(message):
	...
	date = datetime.now(pytz.timezone('Etc/GMT-2')).strftime("%Y-%m-%d %H:%M:%S") # Time read by bot, in GMT-2 time zone.
	...
	max_length = 280 - link_length # Max length of actual message, as the referred tweet takes link_length extra chars.
	...
	if len(mes) >= max_length: # The message (mes) doesn't fit in one tweet.
		...
		tweet(parts[0], image) # Tweets first part without referring to latest item in timeline, and 
		del parts[0] # Deletes already tweeted part of message (mes) (first part, with no reffering)
		...
		for i in parts:
			...
			tweet(f"{i} https://twitter.com/TemeroldFeed/status/{timeline[0].id}", image)

	tweet(mes, image) # Calls tweet function, including potential image

keep_alive() # Runs server ping-receiver
client.run(discord_token) # Runs Discord bot client, indefinitely
